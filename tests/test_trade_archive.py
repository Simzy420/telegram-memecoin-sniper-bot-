...
from src.archives.schema import COMBINED_FILLS_FIELDS, FORBIDDEN_FOLDER_NAMES
...
class TradeArchiveWriterTests(unittest.TestCase):
    ...
    def test_record_fill_updates_csv_and_win_loss(self) -> None:
        self.writer.record_fill(
            {
                ...
                "tx_id": "sig-buy",
                "order_id": "ord-1",
                "guard_checks": {"score": 80, "passed": True},
        ...
        self.assertEqual(json.loads(day["fills"][0]["guard_checks"])["score"], 80)
        ...
        self.assertEqual(day["summary"]["specialists"]["guard"]["fills"], 0)

    def test_utc_midnight_rotates_folder(self) -> None:
        self.writer.ensure_day()
        self.writer.record_event({"specialist": "router", "outcome": "routed", "venue": "jupiter"})
        ...
        self.writer.record_event({"specialist": "guard", "outcome": "checked"})
        ...
        self.assertEqual(first_summary["specialists"]["router"]["events"], 1)
        self.assertEqual(first_summary["specialists"]["guard"]["events"], 0)
        self.assertEqual(second_summary["specialists"]["guard"]["events"], 1)
        self.assertEqual(second_summary["specialists"]["router"]["events"], 0)
    ...
    def test_event_timestamp_selects_folder_not_clock(self) -> None:
        ...
        self.assertTrue((self.tmp / "2026-09-11" / "arbiter" / "events.jsonl").is_file())
        self.assertFalse((self.tmp / "2026-09-12" / "day-summary.json").exists())

    def test_rejects_perp_and_swarm_names(self) -> None:
        for name in ("perp", "swarm", "Perps"):
